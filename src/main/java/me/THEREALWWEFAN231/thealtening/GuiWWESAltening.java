...
public class GuiWWESAltening extends GuiScreen {
	...
	public void initGui() {
		...
		this.apiTokenField = new GuiTextField(0, this.fontRenderer, this.width / 2 - widthOfComponents / 2, this.height / 2 - this.height / 6, widthOfComponents, 20);
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		this.fontRenderer.drawStringWithShadow("WWEs' altening mod", this.width / 2 - this.fontRenderer.getStringWidth("WWEs' altening mod") / 2, this.height / 20, -1);
		if (GuiWWESAltening.paidAPIToken == null) {
			//you are not allowed to change this without crediting THEREALWWEFAN231 or WWE Client
			this.fontRenderer.drawStringWithShadow("Use code wwe for 20% off the altening", this.width / 2 - this.fontRenderer.getStringWidth("Use code wwe for 20% off the altening") / 2, this.height / 20 + 10, -1);
		}

		this.fontRenderer.drawStringWithShadow("API Token", this.width / 2 - this.fontRenderer.getStringWidth("API Token") / 2, this.height / 2 - this.height / 6 - 12, -1);
		this.apiTokenField.drawTextBox();
		this.fontRenderer.drawStringWithShadow("Free account token", this.width / 2 - this.fontRenderer.getStringWidth("Free account token") / 2, this.height / 2 - this.height / 6 + 28, -1);
		this.freeTokenField.drawTextBox();

		this.checkAPITokenButton.drawButton(this.mc, mouseX, mouseY, partialTicks);
		this.useButton.drawButton(this.mc, mouseX, mouseY, partialTicks);

		if (this.responseMessage != null) {
			this.fontRenderer.drawStringWithShadow(this.responseMessage, this.width / 2 - this.fontRenderer.getStringWidth(this.responseMessage) / 2, this.height / 6, -1);
		}
		...
		if (!GuiWWESAltening.altInformation.isEmpty()) {
			for (int i = 0; i < GuiWWESAltening.altInformation.size(); i++) {//we will do this loop just incase & and we index control so yeah
				...
				this.fontRenderer.drawStringWithShadow(string, this.width / 16, (this.height / 6) + ((i + 1) * 10), -1);
			}
		}
	...
	protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
		...
		if (this.checkAPITokenButton.isMouseOver()) {
			...
			if ((this.apiTokenField.getText().isEmpty() || this.apiTokenField.getText().length() == 0) && GuiWWESAltening.paidAPIToken == null) {
				this.responseMessage = TextFormatting.GOLD + "The API Token field is empty";
			} else {

				this.responseMessage = TextFormatting.GOLD + "Trying";
		...
		if (this.useButton.isMouseOver()) {
			...
			if ((this.freeTokenField.getText().isEmpty() || this.freeTokenField.getText().length() == 0) && GuiWWESAltening.paidAPIToken == null) {
				this.responseMessage = TextFormatting.GOLD + "Free account field is empty";
			} else {
	...
	public void checkAPIToken() {
		...
		if (jsonObject.has("premium")) {
			...
			this.responseMessage = TextFormatting.RED + "Not premium you will have to buy Basic/Premium to use the auto generator or go to the free mode";
	...
	public void generateAlt() {
		...
		try {
			...
			e.printStackTrace();
			this.responseMessage = TextFormatting.RED + "Failed to login " + e.getMessage();
			return;
		}
	...
	public ArrayList<String> getLinesFromURL(String urlString) {
		...
		try {
			...
			e.printStackTrace();
			this.responseMessage = TextFormatting.RED + "Error " + e.getMessage();//i dont really know about this
		}
